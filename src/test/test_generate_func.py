...
def forward_huong_2_init(model, input_ids, max_beam_size, max_new_tokens):
    ...
    for block in model.blocks:
        # FIX: dùng list thay vì tuple để nhất quán với model.prefill trả về list
    ...
    with torch.inference_mode():
        ...
        for i, kv in enumerate(present_cache):
            kv_buffers[i][0][:, :, :prompt_len, :].copy_(kv[0].expand(max_beam_size, -1, -1, -1))
            kv_buffers[i][1][:, :, :prompt_len, :].copy_(kv[1].expand(max_beam_size, -1, -1, -1))

    return first_logits[0], kv_buffers, prompt_len

def forward_huong_2_step(model, last_tokens_full, kv_buffers, cache_len):
    # FIX: dùng đúng tên attribute của TransformerModel hiện tại
    # (embed, blocks, lm_head, norm) thay vì tên cũ
    with torch.inference_mode():
        x = model.embed(last_tokens_full)

        for l, block in enumerate(model.blocks):
            x = block.forward(x, kv_buffers[l], cache_len)
    ...
    for _ in range(max_new_tokens - 1):
        ...
        if src != list(range(len(src))):
            ...
            for k_buf, v_buf in kv_buffers:
                # Đúng: clone trước khi copy để tránh aliasing khi beam trùng nguồn
...
ALL_GROUPS = [GROUP_NORMAL, GROUP_NON_EARLY_STOP]

def get_group(name):
    n = is_non_early_stop(name)
    if not n: return GROUP_NORMAL
    elif n: return GROUP_NON_EARLY_STOP
...
# ================= SAVE MARKDOWN =================
import os as _os
_default_md = _os.path.join(_os.path.dirname(__file__), "benchmark_results.md")
MD = _os.environ.get("BENCHMARK_MD_PATH", _default_md)
