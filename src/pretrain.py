...
#     total_steps = (len(train_ds) // accumulation_steps) * pretrain_epochs
#     warmup_steps = (len(train_ds) // accumulation_steps) // 5
...
#                 print(f"\rEpoch {epoch+1}/{pretrain_epochs} - Step {global_step}/{total_steps} - loss: {avg_loss:.4f} - lr: {current_lr:.2e}", end='')
...
#         log_progress(f"Epoch {epoch+1}/{pretrain_epochs} Train Loss: {train_loss:.4f} Val Loss: {val_loss:.4f}")
...
#     total_steps = (len(train_ds) // accumulation_steps) * continued_pretrain_epochs
#     warmup_steps = (len(train_ds) // accumulation_steps) // 5
#     lr_lambda = get_step_lr_lambda(warmup_steps, total_steps)
#     scheduler = optim.lr_scheduler.LambdaLR(optimizer, lr_lambda)
...
#     best_val_loss = float('inf')
#     global_step = 0
#     for epoch in range(continued_pretrain_epochs):
#         model.train()
#         train_loss = 0.0
#         batch_count = 0
#         total_batches = len(train_ds)
...
#             if (batch_idx + 1) % 10 == 0 or (batch_idx + 1) == total_batches:
#                 avg_loss = train_loss / batch_count
#                 print(f"\rEpoch {epoch+1}/{continued_pretrain_epochs} - Step {global_step}/{total_steps} - loss: {avg_loss:.4f} - lr: {current_lr:.2e}", end='')
...
#         log_progress(f"Epoch {epoch+1}/{continued_pretrain_epochs} Train Loss: {train_loss:.4f} Val Loss: {val_loss:.4f}")
